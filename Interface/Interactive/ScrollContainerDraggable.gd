	...
	if ev is InputEventMouseButton:
		if ev.pressed and get_global_rect().has_point(ev.global_position):
			...
			if flick_dur > 0.0:
				...
				var _err = tween.interpolate_method(self, 'set_h_scroll', source.x, target.x, flick_dur, Tween.TRANS_LINEAR, Tween.EASE_OUT)
				_err = tween.interpolate_method(self, 'set_v_scroll', source.y, target.y, flick_dur, Tween.TRANS_LINEAR, Tween.EASE_OUT)
				_err = tween.interpolate_callback(tween, flick_dur, 'queue_free')
				_err = tween.start()
			swiping = false
			swipe_mouse_times = []
